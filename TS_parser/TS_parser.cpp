...
int main(int argc, char *argv[ ], char *envp[ ])
{
  ...
  std::string outputFilename = "PID136";
  ...
  if ((err = fopen_s(&file, fileName, "rb")) != 0) {
      ...
      if (file == NULL) {
          cout << "successfully NOT opened" << endl;
      }
      ...
      PES_Assembler.InitMusic(outputFilename, 136);
      //przy InitVideo nie dzia³a :(
      ...
      while (!feof(file))
      {
          ...
          if (TS_PacketHeader.getSyncByte() == 'G' && TS_PacketHeader.getPID() == 136) {
              ...
              case xPES_Assembler::eResult::AssemblingFinished:
                  fwrite(PES_Assembler.getPacket(), sizeof(char), PES_Assembler.getDataLength(), PES_Assembler.getFileHandler());
                  //cout << "Finished\t";
                  //printf("Finished ");
                  //cout << "PES: Len= " << PES_Assembler.getNumPacketBytes();
                  //cout << " HeadLen= " << PES_Assembler.getHeaderLength();
                  //cout << " DataLen= " << PES_Assembler.getDataLength();
                  break;
              default:
                  break;
              }
